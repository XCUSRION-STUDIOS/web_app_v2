...
export default function LoadCharacter() {
  ...
  // Check if no character is stored (default character exists but user hasn't customized it)
  if (!character || character.username === "z246zx$") {
    return (
      <div
        className="h-screen w-screen bg-cover bg-center flex flex-col items-center justify-center text-white"
        style={{ backgroundImage: `url(${backgroundImage})` }}
  ...
  return (
    ...
    >
      ...
      <div className="space-y-5 w-150 max-w-3xl">
        <div className="p-4 h-35 bg-white/5 backdrop-blur-[2px] rounded-lg shadow-sm flex justify-between items-center">
          <div className="flex flex-col space-y-0.5">
            <h2 className="text-xl font-light tracking-wide font-[Metropolis]">
              {character.username || "Unknown"}
            </h2>
            <p className="text-2xl font-bold">Level: 1</p>
            <p className="text-[0.75em]">Class: N/A</p>
            <p className="text-[0.75em]">Title: N/A</p>
            <p className="text-[0.75em]">Age: {character.age || "Unknown"}</p>
            <p className="text-[0.75em]">
              Character Creation: {new Date(character.createdAt).toLocaleString() || "Unknown"}
            </p>
          </div>
